...
# Modern acme.sh compatibility helpers
function prepare_acme(){
...
EOF
    ...
    if [ ! -d "/usr/src/trojan-cert" ]; then
        ...
        install_acme
        ~/.acme.sh/acme.sh --issue --server letsencrypt --ecc -d $your_domain --nginx
        if test -s /root/.acme.sh/${your_domain}_ecc/fullchain.cer || test -s /root/.acme.sh/$your_domain/fullchain.cer; then
            cert_success="1"
        fi
...
EOF
        ...
        cd /root
        ~/.acme.sh/acme.sh --install-cert --ecc -d $your_domain   \
...
function preinstall_check(){
    ...
    local_addr=`curl -4 -fsS https://ipv4.icanhazip.com | tr -d "\r\n"`
...
function repair_cert(){
    ...
    if [ "$real_addr" == "$local_addr" ] ; then
        prepare_acme
        ~/.acme.sh/acme.sh --issue --server letsencrypt --ecc -d $your_domain --standalone
        ~/.acme.sh/acme.sh --install-cert --ecc -d $your_domain   \
            --key-file   /usr/src/trojan-cert/$your_domain/private.key \
            --fullchain-file /usr/src/trojan-cert/$your_domain/fullchain.cer \
            --reloadcmd  "systemctl restart trojan"
